...
public class PeopleYouMayKnowOneAdapter extends BaseAdapter implements AfHttpResultListener {
	...
	private List<AfMutualFreindInfo> afMutualDataList = new ArrayList<AfMutualFreindInfo>();
...
//	private ListViewAddOn listViewAddOn;
	...
	public PeopleYouMayKnowOneAdapter(List<AfMutualFreindInfo> dataList, Context context ) {
		this.mContext = context;
		setMutualList(dataList);
//		this.listViewAddOn = listViewAddOn;
	...
	public PeopleYouMayKnowOneAdapter(Context context, List<AfFriendInfo> dataList ) {
		this.mContext = context;
...
//		this.listViewAddOn = listViewAddOn;
	...
	public Object getItem(int position) {
		if (isMutualList) {
			return afMutualDataList.get(position);
		} else {
...
//					MobclickAgent.onEvent(mContext, ReadyConfigXML.MAY_T_PF);
					
					AfMutualFreindInfo afMutual = (AfMutualFreindInfo) item;
					Intent action = new Intent(mContext, ProfileActivity.class);
					action.putExtra(JsonConstant.KEY_PROFILE,
							(Serializable) AfMutualFreindInfo
									.mutualFriendToProfile(afMutual));
					// 请求新的profile资料
					action.putExtra(JsonConstant.KEY_FLAG, true);
					action.putExtra(JsonConstant.KEY_AFID, afMutual.afid);
...
//					MobclickAgent.onEvent(mContext, ReadyConfigXML.MAY_T_PF);
	...
	private void bindView(ViewHolder viewHolder, final Object item) {
		String signStr = "";
		String mutualNoStr = "";
		String nameStr = "";
		int age = 0;
		...
		if (isMutualList) {
			AfMutualFreindInfo afMutual = (AfMutualFreindInfo) item;
			signStr = afMutual.sign == null ? mContext.getString(R.string.default_status) : afMutual.sign;
			mutualSize = afMutual.comm_frds == null ? 0 : afMutual.comm_frds.size();
			mutualNoStr = mutualSize + mContext.getString(R.string.mutual_friends);
			nameStr = afMutual.name;
			age = afMutual.age;
			sex = afMutual.sex;
			afid = afMutual.afid;
			sn = afMutual.getSerialFromHead();
			isAdd = afMutual.isAddFriend;
			serverUrl = afMutual.getServerUrl();
			
			PalmchatLogUtils.e("TAG", "----afFriend.type:" + afMutual.type);
			
			if (afMutual.type == AfMutualFreindInfo.AFMOBI_THRID_APP_FACEBOOK) {
				viewHolder.mFbmImg.setVisibility(View.VISIBLE);
			} else {
				viewHolder.mFbmImg.setVisibility(View.GONE);
			}
			
			viewHolder.mRegionLayout.setVisibility(View.GONE);
			viewHolder.mMutualLayout.setVisibility(View.VISIBLE);
			
		} else {
			AfFriendInfo afFriend = (AfFriendInfo) item;
			...
			serverUrl = afFriend.getServerUrl();
			...
			viewHolder.mMutualLayout.setVisibility(View.GONE);
		...
		if (mutualSize != 0) {
			...
			viewHolder.mMutualNoTxt.setText("");
		}
...
//				MobclickAgent.onEvent(mContext, ReadyConfigXML.MAY_ADD_SUCC);
				
				AfFriendInfo tmpFriend;
				if(isMutualList) {
					AfMutualFreindInfo afMutual = (AfMutualFreindInfo) item;
					tmpFriend = AfMutualFreindInfo.mutualFriendToProfile(afMutual);
				} else {
					AfFriendInfo afFriend = (AfFriendInfo) item;
					tmpFriend = afFriend;
				}
				
				MessagesUtils.addStranger2Db(tmpFriend);
				...
				mAfCorePalmchat.AfHttpSendMsg(tmpAfid,
						...
						Consts.MSG_CMMD_FRD_REQ, tmpAfid, PeopleYouMayKnowOneAdapter.this);
			}
		});
		
		//调UIL的显示头像方法, 替换原来的AvatarImageInfo.让图片管理更好
		ImageManager.getInstance().DisplayAvatarImage(viewHolder.mHeadImg, serverUrl,
		...
		AvatarImageInfo imageInfo = new AvatarImageInfo(afid, sn, name, Consts.AF_HEAD_MIDDLE, serverUrl, sex);
	...
	public void setMutualList(List<AfMutualFreindInfo> dataList) {
		this.afMutualDataList.clear();
		if (null != dataList) {
			this.afMutualDataList.addAll(dataList);
		}
	}
	...
	// 刷新
	public void notifyMutualSetChanged(List<AfMutualFreindInfo> dataList) {
		setMutualList(dataList);
			...
			if (code == Consts.REQ_CODE_SUCCESS) {
				if (user_data != null && user_data instanceof String) {
					...
					if (!StringUtil.isNullOrEmpty(afid)) {
						if (isMutualList) {
							if (null != afMutualDataList) {
								int size = afMutualDataList.size();
								for (int i = 0; i < size; i++) {
									AfMutualFreindInfo info = afMutualDataList.get(i);
							...
							if (null != afFriendDataList) {
								int size = afFriendDataList.size();
								for (int i = 0; i < size; i++) {
									AfFriendInfo info = afFriendDataList.get(i);
									if (afid.equals(info.afId)) {
										info.isAddFriend = true;
										break;
									}
			...
			if (code == Consts.REQ_CODE_SUCCESS) {
				if (user_data != null && user_data instanceof String) {
					...
					mAfCorePalmchat.AfHttpFriendOpr("all", afid,
							...
							PeopleYouMayKnowOneAdapter.this);
				}
