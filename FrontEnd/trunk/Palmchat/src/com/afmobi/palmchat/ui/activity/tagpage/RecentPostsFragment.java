...
public class RecentPostsFragment extends BaseFragment implements OnClickListener, IXListViewListener,
        AfHttpResultListener, OnJumpEventListener {
    private final String TAG = RecentPostsFragment.class.getSimpleName();
    /**
     * 列表
     */
     ...
     * 广播列表
     */
    private ArrayList<AfChapterInfo> mAfChapterList = new ArrayList<AfChapterInfo>();
    /**
     * 空视图
     */
    ...
    private void loadData(boolean isLoadMore) {
        ...
        mAfCorePalmchat.AfHttpAfBcgetChaptersByNewTag(mPageId, mCurPageIndex * LIMIT, LIMIT, mCurTagName, this);
    }
    ...
    private void showEmptyView(boolean isIllegal) {
        ...
        if (mTopPostsAdapter.getCount() <= 0) {
            ...
            if (isIllegal) {
                ...
                mTv_HintSecond.setText(activity.getResources().getString(R.string.first_write_something));
    ...
    public void onEventMainThread(ShareBroadcastResultEvent shareBroadcastResultEvent) {
        if (shareBroadcastResultEvent.errorCode == Consts.REQ_CODE_UNNETWORK) {// 如果是无网络的 那就提示网络失败
    ...
    public void onEventMainThread(AfChapterInfo afChapterInfo) {
        ...
        if (afChapterInfo.eventBus_action == Constants.UPDATE_BROADCAST_MSG) {//发广播 更新状态
            ...
            if (afChapterInfo.status == AfMessageInfo.MESSAGE_SENT) {
                if ((!TextUtils.isEmpty(afChapterInfo.content)) && (afChapterInfo.content.contains(mCurTagName))) {
                    CacheManager.getInstance().setBC_RecordSendSuccessData(afChapterInfo);
                    mTopPostsAdapter.updateDataStatus(afChapterInfo);
                    setListScrolltoTop(false);
                    showEmptyView(false);
                }
            } else if (afChapterInfo.status == AfMessageInfo.MESSAGE_UNSENT) {
                mTopPostsAdapter.updateDataStatus(afChapterInfo._id);
                showEmptyView(false);
            } else if (afChapterInfo.status == AfMessageInfo.MESSAGE_SENTING) {
                if ((!TextUtils.isEmpty(afChapterInfo.content)) && (afChapterInfo.content.contains(mCurTagName))) {
                    Activity activty = getActivity();
                    if ((null != activty) && (activty instanceof TagPageActivity)) {
                        ((TagPageActivity) activty).setCurrentItem(1);
                    }
                    mTopPostsAdapter.updateData(afChapterInfo);
                    setListScrolltoTop(false);
                    showEmptyView(false);
                }
            ...
            mTopPostsAdapter.notifyDataSetChanged_removeBymid(afChapterInfo.mid);//modify by wxl 要用mid来删除要删除的广播
